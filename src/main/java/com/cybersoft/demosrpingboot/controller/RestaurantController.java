...
//                    "attachment: filename*\"" + resource.getFilename() + "\"").body(resource);
    ...
    @CacheEvict(value = "getAllRestaurant", allEntries = true)
    @GetMapping("/removecatching")
    public ResponseEntity<?> removeCatching(){
        return ResponseHelper.getResponse("Remove catching is successful",HttpStatus.OK);
    }

    @GetMapping("/details")
    public ResponseEntity<?> getDetailRestaurant(@RequestParam int id){
        RestaurantDto restaurantDto = restaurantImp.getDetailRestaurant(id);
        if (restaurantDto != null) {
            return ResponseHelper.getResponse(restaurantDto,HttpStatus.OK);
        }
        return ResponseHelper.getError("Can not find any restaurant", HttpStatus.NO_CONTENT);
    }
}
